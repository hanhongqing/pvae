...
class DatasetTrainSinglePanVision(torch.utils.data.Dataset):
    def __init__(self, args, stacks, stacks_add1, stacks_add2, how2treatnext1args, sample_stack_wherein, sample_isotropic, sample_continuous, preprocess_patch_reflect, n_replication, fix_r):
        self.behavior = how2treatnext1args #"ignore"/"reuse"/"avoid"
        if self.behavior == "ignore":
            assert n_replication == 1
        self.sample_isotropic  = sample_isotropic
        ...
        self.n2 = np.empty([self.tn_train, self.n_rep, 3], dtype=float)
        #**********
        self.stacks_add1 = stacks_add1
        self.stacks_add2 = stacks_add2
        #assert args.r_nbh < 1.0
        self.add = np.empty([self.tn_train, 2], dtype=float)
        #**********
        ##
        if fix_r:
            r_min = (args.r_train_min+args.r_train_max) / 2
        ...
        for i in range(self.n_stack):
            ...
            #**********
            if self.n_rep == 1:
                ids = self.v[ c:c+args.n_train[i]].squeeze(axis=1).astype(np.uint16)
                self.add[c:c+args.n_train[i],0] = self.stacks_add1[i][ ids[:,0],ids[:,1],ids[:,2] ]
                self.add[c:c+args.n_train[i],1] = self.stacks_add2[i][ ids[:,0],ids[:,1],ids[:,2] ]
            else:
                self.add[c:c+args.n_train[i],0] = 0.0
                self.add[c:c+args.n_train[i],1] = 0.0
            #**********
            c+=args.n_train[i]
        #**********
    ...
    def __getitem__(self,index):
        ...
        for j in range(self.n_rep):
            ...
            if self.preprocess_patch_color_noise:
                ...
                patch = patch + noise
            output[j] = patch[0]
        return output, self.s[i], self.add[i]
